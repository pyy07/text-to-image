...
export class ComicGenerationService {
  ...
  private buildArticleToComicPrompt(
    ...
    return `# ROLE
你是一位专业的漫画分镜与信息图设计师，擅长将长文提炼为一张「漫画总结」信息图。你能够：准确把握文章重点、设计清晰的分镜布局、在图中书写工整易读的中文配文，并保持整张图风格统一、信息密度适中。

# MISSION
将下方文章转化为一页专业、统一的「漫画总结」信息图（类似公众号漫画复盘），输出为一张完整图。只输出这一张图，不要输出文字说明或分步描述。

# GLOBAL STYLE
- 风格：${styleDescription}
- 基调：信息清晰、有吸引力；整张图画风与字感保持一致。
- 元素：每个分镜内须有表情/对话气泡，以及简短的说明文字（可置于格内或格下）。
- 语言：所有文字（气泡、说明、标题）必须为清晰可读的中文。
- 文字书写要求：图中所有中文配文须工整、横平竖直、字形端正不歪扭；笔画清晰、大小一致，确保易读；避免手写潦草或歪斜。
...
- 根据文章要点自行决定分镜数量与每格内容；提炼关键信息，每格对应一个要点，避免堆砌冗长原文。
- 顺序与文章逻辑一致；可含：引入、要点 1～N、结论/影响等。
- 每格内需有简短说明文字（Caption）与/或对话气泡（Dialogue），内容需为中文且与正文相符；文字精炼、一目了然。

# TECHNICAL OUTPUT PARAMETERS
- 输出：单张合成图（仅输出图像，不输出文字说明）。
- 分辨率：最长边不超过 1024 像素（1K）。
- 图中文字：中文配文务必工整端正、横平竖直，不可歪扭或潦草。
...
请根据以上内容直接生成这一张漫画分镜总结图。只输出图像，不要输出任何文字说明。`
  }
}
