...
# MISSION
根据用户描述生成一段可直接使用、观感专业的动画代码。只输出代码本身，不要解释性文字。代码必须完整、可独立运行，且视觉上达到「可直接对外展示」的水准。`
...
const VISUAL_QUALITY_SVG = `# VISUAL QUALITY（视觉与动效，必须满足）
- 配色：使用和谐的主色+辅色（如 2–4 种），避免刺眼或杂乱；背景与前景对比清晰，可选用深色背景+亮色图形或浅色背景+饱和图形。
- 布局：留白适中、元素对齐、视觉重心平衡；重要信息（标题、公式、图例）层次分明。
- 动效：过渡使用 ease 或 ease-in-out，节奏清晰；有明确的阶段性（过程感），入场/展开/收尾可区分；循环衔接自然，避免生硬闪烁。
...
- 根元素为 \<svg\>，需包含 xmlns="http://www.w3.org/2000/svg"，视口合理（如 viewBox="0 0 400 300"）。
...
- 结构：\<html\>/\<head\>/\<body\> 完整，动画元素在 body 内；可选用 \<canvas\> 或 DOM+CSS/JS。
...
# VISUAL QUALITY（视觉与动效，必须满足）
- 配色：主色+辅色和谐（2–4 种），背景与内容对比清晰；避免刺眼或杂乱配色。
- 布局：留白适中、对齐与重心平衡；标题/重点信息层次分明。
- 动效：过渡使用 ease/ease-in-out，有阶段性（过程感）；节奏清晰，循环衔接自然。
